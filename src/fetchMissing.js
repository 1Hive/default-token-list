...
async function getLogo(gnosisAddr) {
  const response = await fetch(
    `https://blockscout.com/poa/xdai/tokens/${gnosisAddr}/token-transfers`
  );
  const text = await response.text();
  const mainnetAddr = text.match(
    /https:\/\/etherscan.io\/token\/(0x[a-fA-F0-9]{40})/
  )[1];
